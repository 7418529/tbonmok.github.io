;window.CloudflareApps=window.Eager=window.CloudflareApps||window.Eager||{};window.CloudflareApps=window.CloudflareApps||{};CloudflareApps.siteId="af146130c678166a0765dfbd3420d5c0";CloudflareApps.installs=CloudflareApps.installs||{};;(function(){'use strict'
...
return false}}());CloudflareApps.installs["piFUpUzpe41B"]={appId:"FKAQLLHf4tr5",scope:{}};;CloudflareApps.installs["piFUpUzpe41B"].options={"backgroundColor":"#171717","behavior":{"outMode":"bounce"},"fewerParticles":false,"interaction":{"onClick":"push","onHover":"repulse"},"particleColor":"#3cffff","showAdvanced":false};;if(CloudflareApps.matchPage(CloudflareApps.installs['piFUpUzpe41B'].URLPatterns)){(function(Math){var trimLeft=/^\s+/,trimRight=/\s+$/,tinyCounter=0,mathRound=Math.round,mathMin=Math.min,mathMax=Math.max,mathRandom=Math.random;function tinycolor(color,opts){color=(color)?color:'';opts=opts||{};if(color instanceof tinycolor){return color;}
...
else{window.tinycolor=tinycolor;}})(Math);};if(CloudflareApps.matchPage(CloudflareApps.installs['piFUpUzpe41B'].URLPatterns)){var pJS=function(tag_id,params){var canvas_el=document.querySelector("#"+tag_id+" > .particles-js-canvas-el")
...
xhr.send()}};if(CloudflareApps.matchPage(CloudflareApps.installs['piFUpUzpe41B'].URLPatterns)){"use strict";(function(){if(!window.addEventListener)return;var _window=window;var tinycolor=_window.tinycolor;var CONTAINER_ID="eager-particles-js";var getComputedStyle=document.defaultView.getComputedStyle.bind(document.defaultView);var options=CloudflareApps.installs['piFUpUzpe41B'].options;var element=void 0;function getParticleColor(){var particleColor=void 0;if(options.particleColor){particleColor=tinycolor(options.particleColor);}else{var backgroundColor=options.backgroundColor||getComputedStyle(document.body).backgroundColor;var components=tinycolor(backgroundColor).toHsl();components.l=Math.abs((components.l+0.5)%1)+(1-components.s)*0.1;particleColor=tinycolor(components);}
...
window.CloudflareApps.installs['piFUpUzpe41B'].scope={setColors:function setColors(nextOptions){options=nextOptions;if(!window.pJSDom){updateElement();return;}
